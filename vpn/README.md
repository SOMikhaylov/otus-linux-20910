...
TAP
 ...
 - Недостатки:
    1. в тоннель попадает broadcast-трафик, что иногда не требуется;
    2. добавляет свои заголовки поверх заголовков Ethernet на все пакеты, которые следуют через тоннель;
    3. в целом, менее масштабируем из-за предыдущих двух пунктов;
    4. не поддерживается устройствами Android и iOS (по информации с сайта OpenVPN).

Если выполнить команду arping на TAP интерфейсе, то она отработает штатно, т.к. TAP эмулирует Ethernet устройство.
```
[vagrant@client ~]$ arping 10.10.10.1 -I tap0
ARPING 10.10.10.1 from 10.10.10.2 tap0
Unicast reply from 10.10.10.1 [8A:90:AF:FA:C1:59]  1.080ms
Unicast reply from 10.10.10.1 [8A:90:AF:FA:C1:59]  1.994ms
```
...
TUN:
  - Преимущества:
    ...
    2. сравнительно (отн. TAP) меньшие накладные расходы и, фактически, ходит только тот IP-трафик, который предназначен конкретному клиенту.
  - Недостатки:
